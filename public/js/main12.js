...
function handleCreate(event) {
  ...
  if (message.value != "") {
    let task = {
      ...
      profileImageUrl:
        "https://apis.tradetipsapp.com/api/appUser/getImageByAppUserId?appUserId=" +
        loggedInVal,
      messageSource: "Web",
      // status: "incomplete"
    };
...
function getoutput(event, id) {
  ...
  const docRefreply = db.collection(
    "/openGroups/roomOne/messages/"+ id + "/replies/"
  );
...
function fetchTasks() {
  ...
  docRef.orderBy("createdDate", "asc").onSnapshot(function(snapshot) {
    snapshot.docChanges().forEach(function(change) {
      ...
      docRefreply
        .orderBy("createdDate", "asc").onSnapshot(function(snapshots) {
          $("#sizedata" + change.doc.id).html(snapshot.size);
...
function fetchTasksReply(id) {
  ...
  var docId = id;
  const docRefreply = db.collection(
    "/openGroups/roomOne/messages/"+ docId + "/replies/"
  );
      ...
      return `
           ...
           <div class="Overlay">
               ...
               <div class="header clearfix">
                 ...
                 <p class='message'><img data-toggle="modal" data-target="#ModalCenter${uniqueId}" src="${message}" onClick='myImgs(this.id)' id="message_img${uniqueId}" class="img-responsive"   style="width:500px;"/> <svg class="Chevron" onclick="TogglePopup(this.id)" id="${taskId}"  xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                 width="24" height="24"
                 viewBox="0 0 172 172"
      ...
      return `
         ...
         <div class="chat-body clearfix agent" style="float:none;background:#77839647;color:#000;">
           <div class="overlay">
             ...
             <p class='message' style="color: #000 !important">
            <imgdata-toggle="modal" data-target="#ModalCenter${uniqueId}" src="${message}" onClick='myImgs(this.id)' id="message_img${uniqueId}" class="img-responsive"   style="width:500px;"/> <img class="chevron" onclick="togglePopup(this.id)" id="${taskId}" src=" https://img.icons8.com/external-royyan-wijaya-detailed-outline-royyan-wijaya/24/000000/external-chevron-arrow-line-royyan-wijaya-detailed-outline-royyan-wijaya.png" />
